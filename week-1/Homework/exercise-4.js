  ...
  */
  ...
  let restaurantFinderApplication = {
    ...
    findAvailableRestaurants: function (numberOfPeople) {
      let available = restaurants.filter(function(item){    //создаем переменную, в которой будет результат
        // Шаг1; фильтруем исходный массив, в соответствии с условиями
        if((item.totalSeats-item.numberOfCustomers) >= numberOfPeople){
          return true;
        }
        //Шаг 2; проходим по результатам фильтрации и возвращаем свойство Элеммента 
         }).map(function(item){   // 
           return item.name;
         })
      // Complete here
    ...
    findRestaurantServingDish: function (dishName) {
      let search = restaurants.filter(function (item){
      if(item.menu.includes(dishName)){
        return true;
      }
      // Complete here
    })
...
return search;},
    countNumberOfRestaurantsInArea: function (area) {
      let searchLoc= restaurants.filter(function(elem){
        if(elem.address.area === area){
        return true;
        }
      });
     return searchLoc.length;
      // Complete here
    },
  };
