...
let destinationNameReachableByFerry =travelDestinations.filter(function(destination){
   ...
   return destination.destinationName;// Complete here
})

let destinationNamesMoreThan300KmsAwayByTrain =travelDestinations.filter(function(destination){
    let isMoreThan300km= destination.distanceKms>300;
    let isReachableByTrain =destination.transportations.includes("train")
    return  isMoreThan300km && isReachableByTrain ;

}).map(function(destination){
    return destination.destinationName; // Complete here (PRINT THE RESULT IN THE CONSOLE USING FOREACH)
})
/*
